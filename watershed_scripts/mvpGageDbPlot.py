...
if watershedName:
	...
	#Calls the JOptionPane funtion:  showInputDialog(Parent Component, Message, Title, MessageType, Icon, List, Initial Selection)
	projectName = JOptionPane.showInputDialog(None,"Choose a Gage","Gage Plot",JOptionPane.PLAIN_MESSAGE,None,projectList, projectList[0])

	if projectName:
		#construct timeseries

		params = dataDict[watershedName][projectName]
		omRespon, datum, poolLevelTsID, poolLevel2TsID, poolLevel3TsID, outflowTsID, publicName, forecastedInflowTsID, tailwaterLevelTsID, stageMarkers, flowMarkers, units, elevationOrStage, usgsID, mvpHydrographer = params
		...
		if poolLevelTsID:
			...
			poolLevelTsIDList = ['{}.Stage.Inst.15Minutes.0.rev'.format(projectName),
						...
						'{}.Stage.Inst.~15Minutes.0.Raw-USGS'.format(projectName)]
		...
		rawLevelTsIDList = ['{}.Stage.Inst.15Minutes.0.CEMVP-GOES-Raw'.format(projectName),
		...
		for rawLevelTsID in rawLevelTsIDList:
			rawLevel = getDataIfExists(rawLevelTsID)
			if rawLevel:
				break		
		...
		for stageMeasTsID in stageMeasTsIdList:
			stageMeas = getDataIfExists(stageMeasTsID)
			if stageMeas:
				print(stageMeas)
				break			
		#get outflow
		if outflowTsID:
			...
			outflowTsIdList = ['{}.Flow.Inst.15Minutes.0.rev'.format(projectName),
						'{}.Flow.Inst.1Hour.0.rev'.format(projectName),
						'{}.Flow.Inst.15Minutes.0.rev-USGS'.format(projectName),
						'{}.Flow.Inst.15Minutes.0.Raw-USGS'.format(projectName),
						'{}.Flow.Inst.~1Day.0.CEMVP-Legacy'.format(projectName),]
		...
		if tailwaterLevel is not None:
			...
			stageTailView = None
		if rawLevel is not None:	
			stageView.addCurve("Y1", rawLevel.getData())
		...
		if poolLevel is not None:
			if elevationOrStage:
				plot.getViewport(poolLevel.getData()).getAxis('Y1').setLabel('Elevation, ft {}'.format(datum))
			else:
				plot.getViewport(poolLevel.getData()).getAxis('Y1').setLabel('Stage Level, ft')
		...
		## Plot Line Styles
		if poolLevel is not None:
			curveFormatter(plot, poolLevel.getData(), "blue", 3, "Stage Rev", None, None)
		if rawLevel is not None:
			curveFormatter(plot, rawLevel.getData(), "lightgray", 3, "Stage Raw", None, None)
		if (stageMeas is not None):
			curve = curveFormatter(plot, stageMeas.getData(), "red", 0, 'Measurement', None, None)
		...
		if (outflowMeas is not None):
			...
			if outflow is not None:
				curveFormatter(plot, outflow.getData(), "lightblue", 1.5, "Discharge", None, None)
		else:
		
			if outflow is not None:
				curveFormatter(plot, outflow.getData(), "red", 1.5, "Discharge", None, None)

		
		if forecastedInflow is not None and outflow is not None:
		...
		del(rawLevel, datum, poolLevelTsID, poolLevel2TsID, poolLevel3TsID, outflowTsID, forecastedInflowTsID, tailwaterLevelTsID, stageMarkers, flowMarkers, units, elevationOrStage)
